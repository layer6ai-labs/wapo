...
def main() -> None:
    ...
    # Map logical GPU indices to physical devices from parent's CUDA_VISIBLE_DEVICES
    parent_devices = os.environ.get("CUDA_VISIBLE_DEVICES")
    if parent_devices:
        available_gpus = [d.strip() for d in parent_devices.split(",")]
    else:
        available_gpus = [str(i) for i in range(num_inference_gpus + num_trainer_gpus)]
    total_needed = num_inference_gpus + num_trainer_gpus
    if len(available_gpus) < total_needed:
        raise SystemExit(
            f"Need {total_needed} GPUs ({num_inference_gpus} inference + {num_trainer_gpus} trainer) "
            f"but only {len(available_gpus)} available: {','.join(available_gpus)}"
        )
    inference_gpu_str = "CUDA_VISIBLE_DEVICES=" + ",".join(
        available_gpus[:num_inference_gpus]
    )
    trainer_gpu_str = "CUDA_VISIBLE_DEVICES=" + ",".join(
        available_gpus[num_inference_gpus : num_inference_gpus + num_trainer_gpus]
    )
